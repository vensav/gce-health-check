...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# google-authenticator related packages
			apt-get install -y libpam-google-authenticator qrencode pamtester
		elif [[ $OS == 'centos' ]]; then
...
if [[ $OS =~ (debian|ubuntu) ]]; then
	...
	# pam.d entry for openvpn
	echo "auth requisite /usr/lib/x86_64-linux-gnu/security/pam_google_authenticator.so secret=/etc/openvpn/google-authenticator/\${USER} user=root forward_pass" >> /etc/pam.d/openvpn
	echo "#@include common-auth uncomment if unix password auth needed besides MFA Code"  >> /etc/pam.d/openvpn
...
function newClient() {
	...
	### Generate Google Authenticator Client Credentials for Ubuntu
	if [[ $OS =~ (debian|ubuntu) ]]; then
		
		## Create a non-login unix user with no password
		adduser --gecos "" --disabled-login ${CLIENT}
		HOST=$(hostname)
		google-authenticator -t -d -f -r 3 -R 30 -W -C -s "/etc/openvpn/google-authenticator/${CLIENT}" || { echo -e "${R}${B}error generating QR code${C}"; exit 1; }
		secret=$(head -n 1 "/etc/openvpn/google-authenticator/${CLIENT}")
		qrencode -t PNG -o "${homeDir}/${CLIENT}.png" "otpauth://totp/${CLIENT}@${HOST}?secret=${secret}&issuer=openvpn" || { echo -e "${R}${B}Error generating PNG${C}"; exit 1; }
		echo ""
		echo "2 Factor Auth Enabled. Import Code from $homeDir/$CLIENT.png."

		echo " Server-Side troubleshooting tips -
			1. In case of errors use tail -100 /var/log/auth.log
			2. Test PAM config with pamtester openvpn ${CLIENT} authenticate
		"
	fi
